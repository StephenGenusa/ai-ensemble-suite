...
async def main():
    ...
    if config_path.exists():
        try:
            config_path.unlink()
            print(f"Removed existing config file to ensure clean workflow: {config_path}")
        except Exception as e:
            print(f"Warning: Could not remove existing config: {e}")
    ...
    try:
        ...
        async with Ensemble(config_dict=WORKFLOW_CONFIG) as ensemble:
            ...
            # Save trace to file
            trace_path = output_dir / "workflow_trace.json"
            try:
                if isinstance(response_data, dict) and 'trace' in response_data:
