        ...
        try:
            ...
            if not self._review_levels:
                 ...
                 for level_idx, level_config in enumerate(self._review_levels):
                    ...
                    try:
                        # Pass all of review_context, which includes 'current_content' and previous inputs/reviews
                        prompt_vars = {**review_context}
                        # Ensure 'current_content' key exists even if empty
                        prompt_vars.setdefault("current_content", "")
                        # Add a specific key for the content being reviewed *in this step* for clarity in templates
                        prompt_vars["content_to_review"] = review_context.get("current_content", "")

                        # Format using BaseCollaborationPhase method
                        review_prompt = self.format_prompt(template_to_use, **prompt_vars)
    ...
    async def _get_initial_content(
        ...
        if draft_template:
            ...
            try:
                ...
                draft_prompt_vars = {"query": query, **inputs}
                # Use format_prompt from Base Collaboration class
                draft_prompt = self.format_prompt(draft_template, **draft_prompt_vars)
