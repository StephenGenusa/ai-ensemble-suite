...
class ExpertCommittee(BaseCollaborationPhase):
    ...
    async def _process_inputs(
        ...
        try:
            ...
            if len(organized_inputs) > 1:
                ...
                prompt_vars = {"query": query, "inputs": combined_input}
                
                # Also add individual inputs
                for phase_name, content in organized_inputs.items():
                    prompt_vars[phase_name] = content
            else:
                # Single input case
                phase_name = next(iter(organized_inputs.keys()), "")
                content = next(iter(organized_inputs.values()), "")
                prompt_vars = {"query": query, "input": content, phase_name: content}
                
            processing_prompt = self.format_prompt(template_name, **prompt_vars)
    ...
    async def _evaluate_inputs(
        ...
        try:
            ...
            eval_prompt = self.format_prompt(eval_template_name, **prompt_vars)
        except (ConfigurationError, KeyError) as e:
    ...
    async def _format_final_output(
        ...
        try:
            ...
            format_prompt = self.format_prompt(format_template_name, **prompt_vars)
        except (ConfigurationError, KeyError) as e:
