...
class AdversarialImprovement(BaseCollaborationPhase):
    ...
    async def _generate_initial_solution(
        ...
        try:
            prompt_vars = {"query": query, **inputs}
            initial_prompt = self.format_prompt(self._initial_template, **prompt_vars)
    ...
    async def _generate_critique(
        ...
        try:
            prompt_vars = {"query": query, "solution": solution}
            critique_prompt = self.format_prompt(self._critique_template, **prompt_vars)
        except (ConfigurationError, KeyError) as e:
    ...
    async def _generate_improvement(
        ...
        try:
            prompt_vars = {"query": query, "solution": solution, "critique": critique}
            improvement_prompt = self.format_prompt(self._improvement_template, **prompt_vars)
        except (ConfigurationError, KeyError) as e:
